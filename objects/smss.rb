...
class WTS::Smss
  ...
  def deliver(phone, msg)
    if @pgsql.exec(
      ...
      [phone]
    )[0]['count'].to_i > 16 && ENV['RACK_ENV'] != 'test'
      raise(WTS::UserError, 'E183: We\'ve sent too many of them already, wait for a few hours and try again')
    end
    if @pgsql.exec(
      'SELECT COUNT(*) FROM sms WHERE created > NOW() - INTERVAL \'4 HOURS\''
    )[0]['count'].to_i > 256 && ENV['RACK_ENV'] != 'test'
