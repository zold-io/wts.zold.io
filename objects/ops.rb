...
class WTS::Ops
  ...
  def remove
    ...
    Zold::Remove.new(wallets: @wallets, log: @log).run(['remove', @item.id.to_s, '--force'])
  end
  ...
  def pull(id = @item.id)
    ...
    begin
      ...
      raise(
        WTS::UserError,
        "E186: We didn't manage to find your wallet #{@user.item.id} " \
    ...
    @log.info("Wallet #{id} pulled successfully in #{Zold::Age.new(Time.now)}")
  end
  ...
  def push
    ...
    unless @wallets.acq(id, &:exists?)
      raise(
        WTS::UserError,
        "EThe wallet #{id} of #{@user.login} is absent, can't PUSH; " \
    ...
    @log.info("Wallet #{id} pushed successfully in #{Zold::Age.new(Time.now)}")
  end

  def pay_taxes(keygap)
    raise(RuntimeError, "The user #{@user.login} is not registered yet") unless @item.exists?
    raise(RuntimeError, "The account #{@user.login} is not confirmed yet") unless @user.confirmed?
    if @user.fake?
    ...
    unless @wallets.acq(id, &:exists?)
      return if ENV['RACK_ENV'] == 'test'
      raise(RuntimeError, 'There is no wallet file after PULL, can\'t pay taxes')
    end
    Tempfile.open do |f|
    ...
    @log.info("Taxes paid for #{id} in #{Zold::Age.new(Time.now)}")
  end
  ...
  def pay(keygap, bnf, amount, details)
    ...
    unless @wallets.acq(id, &:exists?)
      return if ENV['RACK_ENV'] == 'test'
      raise(RuntimeError, 'There is no wallet file after PULL, can\'t pay')
    end
    txn =
      Tempfile.open do |f|
        File.write(f, @item.key(keygap))
    ...
    @log.info("Paid #{amount} from #{id} to #{bnf} in #{Zold::Age.new(Time.now)} #{details.inspect}: #{txn.to_text}")
    txn
  end
  ...
  def migrate(keygap)
    ...
    push
    @log.info(
      "Wallet of #{@user.login} migrated from #{@user.item.id} to #{target} " \
      "with #{balance}, in #{Zold::Age.new(Time.now)}"
    )
  end
end
