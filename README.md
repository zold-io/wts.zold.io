...
Later, you can check the status of the job via the `/job.json`
entry point, using its ID.
...
* `GET /find`: finds and returns all transactions in the wallet
...
* `GET /job`: checks the status of the jobs, expecting `id` as a query argument.
  ...
  Returns `200` and a full stack trace as plain text if the job is finished
  with an exception.
  Returns `404` if there is no such job.
  You may also want to use `/job.json` to get more information, in JSON.

* `GET /output`: returns the entire log of a particular job,
  expecting `id` as a query argument;
  the HTTP header `X-Zold-JobStatus` will contain
  either `OK`, `Running`, or `Error`,
...
(your wallet, if not provided)
* `prefix`: the prefix you expect them to arrive to
(get it at `/invoice.json` first)
* `regexp`: the regular expression to match payment details, e.g. `pizza$`
(the text has to end with `pizza`)
...
* `repeat`: set to `true` if you want it to re-create itself
